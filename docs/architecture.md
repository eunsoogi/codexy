...
| `orchestration`                | Keep     | Use when classifying workflow, surface, and risk or coordinating ownership, goals, agents, threads, worktrees, reviews, compaction, and handoff; load only applicable authorities. | Owns classification, the execution loop, routing boundaries, tool evidence, budgets, compact event deltas, and the final reviewer gate.                                                                                                           |
...
required Watcher server in the corresponding core manifest.
...
| `watcher`   | `{"command":"uv","args":["run","--no-project","--script","./mcp/codexy_mcp_bootstrap.py","watcher","--stdio"],"cwd":"."}` | The metadata-driven bootstrap reads the selected plugin release and starts the required Watcher runtime as a local stdio child process. | `watcher_open`, `watcher_health`, `watcher_cancel`, `watcher_status`, and `watcher_list` expose the required native Codex observation boundary. |
