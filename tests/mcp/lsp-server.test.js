...
test("LSP operations preserve bounded caller timeouts", async () => { await withLspClient(assert, lspServer, repoRoot, async (client) => { const payload = await toolPayload(client, "lsp_document_symbols", { path: externalFixture, timeoutMs: 100, server: { id: "fake-lsp", command: fakeLspCommand() } }); assert.equal(payload.status, "error"); assert.match(payload.reason, /Timed out waiting for textDocument\/documentSymbol/); }, { env: fakeEnv({ CODEXY_FAKE_LSP_INITIALIZE_DELAY_MS: "150", CODEXY_FAKE_LSP_REQUEST_DELAY_MS: "300" }) }); });
