...
fn session_start_context_json() -> &'static str {
    r#"{"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":"For compacted or resumed context hygiene, use $dreaming before continuing. Use Codexy codegraph MCP before direct file reads; include codegraph findings; record codegraph unavailable/uncallable fallback evidence; record registered-but-uncallable/unavailable-tool evidence. Use Codexy LSP; run lsp_status; record unavailable/not applicable evidence. Run hooks/codexy-readiness-guard.sh --check-pr-labels --pr-state-file pr-state.json with repositoryLabels before PR readiness. Run scripts/validate-plugin-config --check-completion-handoff for completion handoff claims. Use hooks/codexy-readiness-guard.sh --check-pr-title and hooks/codexy-readiness-guard.sh --check-merge-message --expected-pr PR_NUMBER before readiness."}}"#
}
