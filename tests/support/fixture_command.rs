...
impl FixtureCommand {
    ...
    fn from_command(mut command: Command, uses_posix_paths: bool, program: &OsStr) -> Self {
        receipt::configure_command(&mut command, program, |directory| {
            if uses_posix_paths {
    ...
    pub(crate) fn output(&mut self) -> std::io::Result<Output> {
        let started_epoch_us = receipt::epoch_microseconds();
        let started = Instant::now();
        let output = self.command.output();
        receipt::write_from_command(
            &self.command,
            &output,
            started_epoch_us,
            receipt::epoch_microseconds(),
            started.elapsed(),
        );
        output
    }
...
impl From<Command> for FixtureCommand {
    fn from(command: Command) -> Self {
        Self::from_command(command, false, OsStr::new(""))
    }
}
