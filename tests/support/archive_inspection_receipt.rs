...
pub(crate) fn configure_command<F>(command: &mut std::process::Command, program: &OsStr, path: F)
where
...
pub(crate) fn write_from_command(
    ...
    let Some(directory) = command_environment(command, DIRECTORY_ENV).map(PathBuf::from) else {
        return;
    };
    let (Some(id), Some(test)) = (
        command_environment(command, ID_ENV),
        command_environment(command, TEST_ENV),
    ) else {
        return;
    };
    ArchiveInspectorReceipt {
        directory,
        id: id.to_string_lossy().into_owned(),
        test: test.to_string_lossy().into_owned(),
    }
    .write(output, started_epoch_us, ended_epoch_us, duration);
}
...
fn command_environment(command: &std::process::Command, key: &str) -> Option<OsString> {
