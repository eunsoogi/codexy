...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut client = search_client()?;
    let first_text = search_text(&mut client, root.path(), "ENTRY", 10)?;
    let second_text = search_text(&mut client, root.path(), "ENTRY", 10)?;
    ...
    let missing = search(&mut client, root.path(), "MISSING", 10)?;
    ...
    let result_limited = search(&mut client, root.path(), "ENTRY", 1)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut client = search_client()?;
    let text = search_text(&mut client, root.path(), "ENTRY_MEGABYTE", 1)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut client = search_client()?;
    let result = search(&mut client, root.path(), "NEEDLE", 1)?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, bytes) in [("below", MATCH_LIMIT_BYTES - 1), ("exact", MATCH_LIMIT_BYTES), ("above", MATCH_LIMIT_BYTES + 1)] {
        ...
        let result = search(&mut client, root.path(), &format!("ENTRY_LINE_{name}"), 1)?;
    ...
    let result = search(&mut client, root.path(), "ENTRY_UTF8", 1)?;
...
fn codegraph_search_honors_total_content_byte_boundaries() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base_result = search(&mut client, base.path(), "ENTRY_TOTAL", 10)?;
    ...
    for (name, adjustment, truncated) in [("below", -1_isize, false), ("exact", 0, false), ("above", 1, true)] {
        ...
        let result = search(&mut client, root.path(), "ENTRY_TOTAL", 10)?;
        assert!(
...
fn search(
    ...
    Ok(serde_json::from_str(&search_text(client, root, query, limit)?)?)
}
...
fn search_client() -> Result<McpClient, Box<dyn std::error::Error>> {
