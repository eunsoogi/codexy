...
fn launcher() -> std::path::PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("scripts/profile_rust_windows_launcher.py")
}
...
fn job_owns_immediate_spawn_before_root_returns() -> Result<(), Box<dyn std::error::Error>> {
    let profile = Path::new(env!("CARGO_MANIFEST_DIR")).join("scripts/profile-rust-tests");
    let probe = r#"
