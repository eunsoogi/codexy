...
use crate::{FixtureCommand, archive_inspection_receipt as receipt};
...
impl FixtureCommand {
    ...
    pub fn stdin(&mut self, config: Stdio) -> &mut Self {
        self.command.stdin(config);
        self
    }

    pub fn stdout(&mut self, config: Stdio) -> &mut Self {
        self.command.stdout(config);
        self
    }

    pub fn stderr(&mut self, config: Stdio) -> &mut Self {
    ...
    pub fn output(&mut self) -> std::io::Result<Output> {
        let interval = crate::profile_interval_metrics::command_interval_at(
            OUTPUT_INTERVAL,
            self.command_family,
        ...
        crate::profile_metrics::record_command_wait(
            OUTPUT_KEY,
            self.command_family,
            started.elapsed(),
        );
    ...
    fn measure<T>(
        ...
        let interval =
            crate::profile_interval_metrics::command_interval(interval_key, self.command_family);
        let started = Instant::now();
        let result = invoke(&mut self.command);
        crate::profile_metrics::record_command_wait(key, self.command_family, started.elapsed());
        drop(interval);
        result
    }
}
