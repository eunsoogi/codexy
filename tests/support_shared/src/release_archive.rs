use crate::{FixtureCommand, FixtureHost, wrapper_copy::is_generated_fixture_directory};
pub mod archive_entry;
pub mod archive_evidence;
mod archive_process;
pub use archive_process::{create_archive, create_archive_with_commands};
pub fn inspect_archive(
    host: &impl FixtureHost,
    archive: &std::path::Path,
    plugin_root: &std::path::Path,
    path: Option<&std::path::Path>,
) -> std::io::Result<std::process::Output> {
    let mut command = FixtureCommand::new(
        host.manifest_dir().join("scripts/inspect-release-archive"),
        host.validate_config_binary().as_os_str(),
    );
...
pub fn assert_structured_literals(text: &str, rule_id: &str, required: &[&str]) {
...
pub fn assert_structured_absent_literals(text: &str, rule_id: &str, forbidden: &[&str]) {
...
pub fn assert_archive_scanner_contract(script: &str, entries: &str, checker: &str) {
...
pub fn assert_runtime_workflow_contract(workflow: &str, archive_inspector: &str) {
    ...
    for entry in std::fs::read_dir(source)? {
        ...
        if source_path.is_dir() {
            if entry.file_name() != "runtime" && !is_generated_fixture_directory(&source_path) {
                copy_tree(host, &source_path, &target_path)?;
            }
        } else {
            std::fs::copy(&source_path, &target_path)?;
            if std::fs::read(&source_path)?.starts_with(b"#!") {
                host.make_executable(&target_path)?;
            }
        }
    }
    Ok(())
}
pub fn governed_archive_mode(
    is_windows: bool,
...
pub fn fixture_host_platform(os: &str, architecture: &str) -> std::io::Result<&'static str> {
...
pub fn complete_plugin_fixture(
    host: &impl FixtureHost,
    root: &std::path::Path,
) -> std::io::Result<std::path::PathBuf> {
    complete_plugin_fixture_with_runtime(host, root, true)
}
pub fn complete_plugin_fixture_with_stubbed_runtime(
    host: &impl FixtureHost,
    root: &std::path::Path,
) -> std::io::Result<std::path::PathBuf> {
    complete_plugin_fixture_with_runtime(host, root, false)
}
fn complete_plugin_fixture_with_runtime(
    ...
    copy_tree(
        host,
        &host.manifest_dir().join("plugins/codexy"),
        &plugin_root,
    )?;
    host.materialize_admission_runtime_suite(&plugin_root)?;
    ...
    for (server, binary) in [
        ("lsp", host.mcp_lsp_binary()),
        ("codegraph", host.mcp_codegraph_binary()),
    ] {
