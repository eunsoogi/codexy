...
fn governed_universe(root: &Path) -> Result<BTreeSet<String>, Box<dyn std::error::Error>> { let mut governed = BTreeSet::new(); for path in [".agents/skills","plugins/codexy/hooks","plugins/codexy/skills","plugins/codexy/agents","plugins/codexy/mcp","plugins/codexy/lsp","assets","plugins/codexy/assets","scripts","src/validation","tests",".github/workflows","src/codegraph","src/lsp","src/version","src/bin","packages/getcodexy"] { governed.extend(files(root.join(path))?); } for path in ["src/mcp.rs","Cargo.toml","Cargo.lock","rust-toolchain.toml","rustfmt.toml","clippy.toml","plugins/codexy/bootstrap-codexy-agents","plugins/codexy/check-codexy-agents","plugins/codexy/runtime-release.json",".agents/plugins/marketplace.json",".agents/plugins/release-publish-contract.json",".agents/plugins/runtime-activation.json","plugins/codexy/.codex/lsp-client.json","plugins/codexy/.codex-plugin/plugin.json","plugins/codexy/agents/openai.yaml","README.md","README.ko.md"] { governed.extend(files(root.join(path))?); } Ok(governed) }
