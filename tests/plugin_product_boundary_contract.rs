...
fn validate_records(root: &Path, records: &[SurfaceRecord]) -> TestResult {
    ...
    for record in records { for source in &record.sources { if source.ends_with(".toml") && agent_requires_github_skill(root, source)? && record.target == "codexy" { return Err(format!("core agent requires GitHub skill: {source}").into()); } } }
    Ok(())
}
...
fn agent_requires_github_skill(root: &Path, source: &str) -> Result<bool, Box<dyn std::error::Error>> {
    ...
    Ok(agent.get("developer_instructions").and_then(toml::Value::as_str).is_some_and(|instructions| instructions.split_whitespace().any(|word| word.trim_matches(|character: char| !character.is_ascii_alphanumeric() && character != '-').eq("git-workflow"))))
}
...
fn record_matrix() -> BTreeMap<&'static str, (&'static str, &'static str)> { BTreeMap::from([("hooks.instruction",("codexy","retain")),("hooks.github",("codexy-github","move")),("hooks.policy-core",("codexy","retain")),("hooks.policy-inventory",("remove","remove")),("skills.core",("codexy","retain")),("skills.github",("codexy-github","move")),("skills.repository",("repository-only","move")),("skills.frontend-design",("remove","remove")),("agents.specialists",("codexy","retain")),("agents.github",("codexy-github","move")),("mcp.codegraph",("codexy-devtools","move")),("mcp.lsp",("codexy-devtools","move")),("mcp.grep-app",("remove","remove")),("mcp.runtimes",("codexy-devtools","move")),("lsp.integration",("codexy-devtools","move")),("runtime.codegraph",("codexy-devtools","move")),("runtime.lsp",("codexy-devtools","move")),("runtime.entrypoints",("codexy-devtools","move")),("runtime.repository",("repository-only","split")),("assets.repository",("repository-only","retain")),("assets.plugin",("codexy","retain")),("repository.governance",("repository-only","retain")),("repository.workflows",("repository-only","retain")),("repository.packaging",("repository-only","split")),("public.core",("codexy","retain")),("public.repository",("repository-only","retain"))]) }
fn governed_universe(root: &Path) -> Result<BTreeSet<String>, Box<dyn std::error::Error>> { let mut governed = BTreeSet::new(); for path in ["plugins/codexy/hooks","plugins/codexy/skills","plugins/codexy/agents","plugins/codexy/mcp","plugins/codexy/lsp","assets","plugins/codexy/assets","scripts","src/validation","tests",".github/workflows","src/codegraph","src/lsp","src/version","src/bin","packages/getcodexy/src","packages/getcodexy/tests"] { governed.extend(files(root.join(path))?); } for path in ["src/mcp.rs","Cargo.toml","Cargo.lock","rust-toolchain.toml","rustfmt.toml","clippy.toml","packages/getcodexy/pyproject.toml","plugins/codexy/bootstrap-codexy-agents","plugins/codexy/check-codexy-agents","plugins/codexy/runtime-release.json",".agents/plugins/marketplace.json",".agents/plugins/release-publish-contract.json",".agents/plugins/runtime-activation.json","plugins/codexy/.codex/lsp-client.json","plugins/codexy/.codex-plugin/plugin.json","plugins/codexy/agents/openai.yaml","README.md","README.ko.md"] { governed.extend(files(root.join(path))?); } Ok(governed) }
fn files(path: PathBuf) -> Result<Vec<String>, Box<dyn std::error::Error>> { if path.is_file() { return Ok(vec![repository_identity(Path::new(env!("CARGO_MANIFEST_DIR")), &path)?]); } let mut found = Vec::new(); for entry in std::fs::read_dir(path)? { found.extend(files(entry?.path())?); } Ok(found) }
