...
#[test]
fn validator_rejects_combined_active_waiting_child_thread_count_over_cap()
...
"#,
    ...
    assert!(
        !output.status.success(),
        "validator should aggregate active and waiting child-thread ledger counts"
    );
    assert!(
        String::from_utf8_lossy(&output.stderr)
            .contains("keep at most five active child Codex threads"),
        "stderr should name combined active/waiting over-capacity, got:\n{}",
        String::from_utf8_lossy(&output.stderr)
    );
    Ok(())
}
...
#[test]
fn validator_rejects_same_line_active_waiting_child_thread_count_over_cap()
-> Result<(), Box<dyn std::error::Error>> {
    let output = run_ownership_validator(
        r#"Owner decision: parent-owned for orchestration only; child routing required
...
"#,
    ...
    assert!(
        !output.status.success(),
        "validator should aggregate same-line active and waiting child-thread counts"
    );
    assert!(
        String::from_utf8_lossy(&output.stderr)
            .contains("keep at most five active child Codex threads"),
        "stderr should name same-line active/waiting over-capacity, got:\n{}",
        String::from_utf8_lossy(&output.stderr)
    );
    Ok(())
}
