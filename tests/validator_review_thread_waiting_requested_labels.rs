...
#[test]
fn validator_allows_contracted_requested_label_negations_before_waiting_evidence() -> TestResult {
    for label in [
        "PR readiness: isn't requested",
        "PR readiness: aren't requested",
    ] {
        ...
        assert_success(
            &output,
            "validator should recognize contracted requested-label negations",
        );
    }
    Ok(())
}
...
fn validator_rejects_affirmative_requested_readiness_label_with_waiting_thread() -> TestResult {
    ...
    assert_failure_contains(
        &output,
        "validator should preserve affirmative requested readiness labels",
        "PRRT_kwDOWaiting",
    );
    Ok(())
}
