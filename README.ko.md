...
리뷰, 안전한 완료까지 이어 주는 Codex 하네스입니다. 이 README는 처음 보는 분을
위한 설명이면서 전체 제품과 컴포넌트 범위를 한눈에 보여 주는 공개 개요입니다.
상세 아키텍처와 실행 계약은 연결된 `docs` 문서에서 다룹니다.
...
> **릴리스 상태:** component-aware 1.4.0 제품은 이 저장소에서 구현과 검증을
> 마쳤지만, 1.4.0은 아직 tag로 만들거나 공개하지 않았습니다. 아래 명령은 해당
> 릴리스와 맞는 CLI를 설명하므로, 이 README만 보고 현재 PyPI나 marketplace에서
> 바로 설치할 수 있다고 판단하지 마세요.

### 기본 설치

해당 릴리스가 공개되면 Codexy의 전체 제품을 설치합니다.
...
- **Profile과 전문 에이전트.** 범위가 분명한 작업을 Architect, Cartographer,
  Auditor, Shipwright, Warden, Inspector, Sentinel에 배정하고 GitHub 컴포넌트에는
  Weaver를 추가합니다. 표준 리뷰는 Inspector, 엄격 리뷰는 Sentinel이 담당합니다.
...
플러그인 manifest가 명시하는 플랫폼은 macOS ARM64와 Linux x86_64입니다. CI는
Ubuntu와 macOS에서 패키지를 build·install하고 컴포넌트 lifecycle과 정확한
legacy-to-split candidate 이전을 검증합니다. Native Windows CI는 컴포넌트 CLI,
transaction lifecycle, recovery, GitHub activation 계약을 실행하지만 자동
legacy tree 탐색이나 패키지에 포함된 devtools runtime까지 지원한다고 주장하지
않습니다. LSP 등록을 실제로 쓰려면 host에 해당 language-server 실행 파일도
있어야 합니다.
