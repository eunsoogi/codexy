...
specialist agent가 처음 필요한 Codexy 워크플로에서는 설치된 플러그인의 등록
부트스트랩을 실행합니다. `RESTART_REQUIRED`가 표시되면 specialist 작업 전에 새
작업을 한 번 열면 되며, 저장소 설정 명령을 직접 찾아 실행할 필요는 없습니다.
Codexy는 MCP 바이너리를 소스 플러그인에 넣지 않고 일치하는 GitHub Release
자산에서 런타임을 부트스트랩합니다.
