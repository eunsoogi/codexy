...
마켓플레이스 패키지 워크플로는 `plugins/codexy/**` 아래의 모든 파일,
마켓플레이스 매니페스트 또는 릴리스 게시 계약이 변경될 때 실행됩니다.
패키지를 빌드하는 런타임 소스와 릴리스 스크립트의 변경도 포함됩니다.
관련 없는 문서, 테스트 및 기타 저장소 경로의 변경만으로는 이러한 패키지
입력 중 하나가 함께 변경되지 않는 한 런타임 패키지 빌드가 실행되지 않습니다.
