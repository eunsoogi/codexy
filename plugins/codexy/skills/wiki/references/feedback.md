...
MUST use this reference for `feedback`, `user feedback`, `capture the correction`,
...
preferences, and validation that can influence future sessions.

## Signal Taxonomy

| Signal | Value | Capture? | Notes |
|---|---:|---|---|
| Correction/redirection | high | yes | "wrong", "actually", "stop doing that", "use X instead" |
| Preference/rule | high | yes | "I prefer", "default rule", "always", "stop doing X", "next time" |
...
MUST capture a candidate when a user turn contains one of these durable signals:
...
MUST NOT capture every short acknowledgement. If the only text is `ok`, `thanks`,
...
but feedback capture alone MUST NOT rewrite AGENTS.md or skills.
...
- MUST keep generic acknowledgements out of durable memory.
- MUST require explicit promotion into topic wikis.
- MUST treat approval as contextual validation, not a global fact.
