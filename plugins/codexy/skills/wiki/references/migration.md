...
1. Read existing frontmatter and record the topic root, source paths, and
   current index state in a new `log.md` entry.
2. Keep each existing raw file byte-for-byte. If a source changed, ingest a new
   immutable revision instead of modifying historical raw material.
3. Add missing derived frontmatter only when its value is already present in
   supported data. Rebuild indexes from frontmatter after the source-of-truth
   Markdown is valid.
4. Preserve every complete relative `sources:` scalar exactly. If a source
...
5. Recompile only the affected articles, then query through the normal bounded
   index-and-article path. Record the byte accounting and freshness result.
