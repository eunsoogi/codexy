...
MUST act only when the user supplies one proposal, its authoritative constraints,
and explicit opt-in to frame-check. Invoking `$frame-check` explicitly counts as
opt-in. Otherwise MUST decline without producing a receipt.
