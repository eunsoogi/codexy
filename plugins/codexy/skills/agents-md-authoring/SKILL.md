...
MUST write AGENTS.md files as scoped operating instructions for agents working
in a directory tree. MUST keep them current, actionable, and easy to reconcile
with deeper instructions and higher-priority user or system directions.
...
4. MUST draft instructions that are concrete and durable:
   ...
   - MUST NOT include secrets, local-only paths, temporary notes, or duplicated
     workflow rules that already live in a canonical skill or doc.
...
- MUST NOT claim the instruction update is complete until the final file has
  been reread and its scope, precedence, and verification expectations are
  clear.
