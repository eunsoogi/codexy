...
- Narrow stale-blocked-goal exception: `$goal-state-recovery` MAY use
  `update_goal(status="complete")` only to terminate a stale `blocked`
  execution record after its exact recovery sequence observes that state. This
  is an administrative control-plane unblock, not evidence that an issue, PR,
  ...
  refuses that transition solely because the unfinished blocked objective
  cannot be marked complete, `$goal-state-recovery` MAY use its bounded
  same-directory fork, original-task archive, and forked `get_goal=null`
  fallback to recover control. That fallback is also administrative only and
  MUST NOT apply to timeout, permission, transport, or ambiguous errors.
