...
1. Restate the requested outcome and make a finite list only from its explicit
   requirements, named files, commands, external states, and deliverables.
2. For each item, name the evidence that would prove it. Use file content or
   diff for files, parsers for structured data, tests for executable behavior,
   and the authentic CLI, GitHub, browser, desktop, plugin, marketplace, or
   release surface for externally observable claims.
3. Inspect the current authoritative state. Current head and current external
   state MUST win over memory, intent, plans, and output from older revisions.
