...
interrupts the active local wait MUST be consumed in that same local turn before
the cursor-based wait continues. If a slingshot-host turn still returns
...
instead of repeated model continuations or ending without a wakeup path. The
...
`goal-transition-reporting.md`, complete the finite goal, and leave the task
idle without claiming the issue or implementation complete. A qualifying event
