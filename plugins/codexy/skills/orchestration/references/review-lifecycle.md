Reviewer MUST use the current diff/head. `PASS`, `BLOCK`, and `UNOBSERVABLE`
are terminal; `PENDING` and `RUNNING` are non-terminal observations of the same
...
`previous_control_state` input MUST be rejected. The first full event MUST append
to a clean zero-count genesis; each later state MUST preserve the exact prior
history prefix and increase the terminal count by one. A fresh one-event input
MUST NOT reset prior terminal history, and the validator MUST preserve the
current snapshot's authenticated head and base values.

After full and delta are both consumed, exactly one third `required_current_head`
review may be admitted when the current head moved for mandatory base
integration or an in-scope contract/root repair. It MUST use the same selected
reviewer and carry a typed `post_cap_re_review` reason plus the prior delta head.
...
`baseRefOid`, require a prior `BLOCK` delta with non-empty findings, and bind
`finding_ids` exactly to those findings. Optional churn, duplicate or unchanged heads,
missing/reordered/truncated history, and a fourth terminal verdict MUST be rejected. A third `BLOCK` permits
