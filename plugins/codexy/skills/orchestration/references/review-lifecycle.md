...
existing legacy path. Use it only when an explicit requirement or a concrete
...
`previous_control_state` remains rejected. Missing or incomplete source evidence
stays unknown and MUST NOT be replaced with a fabricated verdict.
...
review-thread, LOC, and merge gates. A third `BLOCK` or `UNOBSERVABLE` may use
the existing bounded disposition only when its authenticated source contract is
satisfied; it MUST NOT create a synthetic `PASS` or a fourth reviewer. See
[review profiles](review-profiles.md) and
