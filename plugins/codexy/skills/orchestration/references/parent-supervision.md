...
through Codex app tasks, a Watcher, Worker callbacks, drift
...
- The Worker is a separate app task that owns the
  implementation branch, files, verification, and its finite execution goal.
...
- Workers and an authorized Watcher MUST be independent Codex
...
- When a material signal arrives, the Orchestrator MUST send one grouped, actionable
  ...
  result. A cooperative freeze-and-report instruction does not cancel an
  in-flight tool unless the host proves a hard stop.
...
- A user or Orchestrator MAY explicitly authorize a same-project Watcher to carry
  the exact long-lived release goal. The Watcher MUST preserve the exact objective
...
- Outside this arrangement, the existing finite execution
  ...
  unresolved; it MUST NOT promise that an idle Orchestrator will wake later.
- In this arrangement, a qualifying event MUST be handled without creating or
  recreating an Orchestrator goal; the Orchestrator MUST return control
  after the authorized event work. This exception changes only the Orchestrator
