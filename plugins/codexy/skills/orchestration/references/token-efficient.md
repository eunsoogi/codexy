...
ledger polling remains permitted. Both the child owner and the root orchestrator
...
5. **Fail once**: a failed parent message MUST emit exactly one terminal
...
8. **Suppress unchanged continuation turns**: when an authorized child-local
   ...
   when an explicit parent/user message arrives. This rule MUST NOT terminate or
...
terminal handoff delta to the source parent. `update_goal(blocked)` additionally
...
artifacts or reservation, and one parent-owned next action. The child MUST
...
For an eligible external gate that outlives the current turn, parent
orchestrators and child owners MUST follow `$orchestration`'s runtime-heartbeat
...
MUST wake the owner only for a material gate change or an explicit user/parent
message. A stable event identity MUST deduplicate repeated wakeups before the
...
increase floors closed. #606 owns each final adoption decision.
