...
TOOLS = frozenset({"mcp__codex_apps__github_create_issue", "mcp__codex_apps__github_update_issue"})
...
def main() -> int:
    ...
    output = evaluate(event, sys.stdin.buffer.read(1024 * 1024 + 1), TOOLS, "CODEXY_REPOSITORY_ISSUE_", forbidden)
    if output:
