...
TOOLS = frozenset({"mcp__codex_apps__github_create_pull_request", "mcp__codex_apps__github_update_pull_request"})
...
def main() -> int:
    ...
    output = evaluate(event, sys.stdin.buffer.read(1024 * 1024 + 1), TOOLS, "CODEXY_REPOSITORY_PULL_REQUEST_", forbidden)
    if output:
