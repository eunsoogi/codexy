...
def evaluate(
    ...
    if invocation.operation == "config":
        ...
        if remote is not None and len(invocation.arguments) == 2 and invocation.arguments[1] and not any(
            ...
            return False, (remote.group(1), remote.group(2).casefold(), invocation.arguments[1]), None
    ...
    if invocation.operation == "remote" and invocation.arguments[:1] in (["add"], ["set-url"]):
        if len(invocation.arguments) != 3 or not invocation.arguments[1] or any(
            char in invocation.arguments[1] + invocation.arguments[2] for char in "\0\r\n"
        ):
            return True, None, None
    ...
    denied = (
        invocation.operation == "reset" and "--hard" in arguments
    ) or (
