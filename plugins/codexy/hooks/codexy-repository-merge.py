...
TOOLS = frozenset({"mcp__codex_apps__github_merge_pull_request", "mcp__codex_apps__github_enable_auto_merge"})
...
def main() -> int:
    ...
    output = evaluate(event, sys.stdin.buffer.read(1024 * 1024 + 1), TOOLS, "CODEXY_REPOSITORY_MERGE_", forbidden)
    if output:
