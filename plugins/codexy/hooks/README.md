...
specialists or the explicit read-only `explorer` role. Generic, worker,
...
tooling MUST be reported as a blocker. The upstream orchestration resolver
selects generic, explicit, fallback, and named-role pairs; the native host owns
the callable pair, and this hook does not infer or authenticate route
provenance. The installed Codexy GitHub plugin activates generic GitHub-command
and destructive shell/Git admission through its own `${PLUGIN_ROOT}` hook
manifest. A trusted repository's `.codex/hooks.json` activates only its
