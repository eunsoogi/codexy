...
Six ordered concerns cover thread-delivery metadata, repository issue changes,
repository pull-request changes, repository merge prevention, GitHub commands,
and destructive shell/Git commands. `PreToolUse` and `PermissionRequest` bind
the same concern owners; matching handlers are independent and any denial is
conservative. Other repositories and unrelated tools pass without context.
Malformed owned input fails visibly rather than guessing.
