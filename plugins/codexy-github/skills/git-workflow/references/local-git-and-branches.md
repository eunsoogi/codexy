...
required starting ref, and verify the requested task branch is not already
owned locally, remotely, or by another PR. Create the task branch only after an
issue or explicit issue-sized scope exists, and keep it in an isolated
worktree. MUST NOT implement directly on the default branch.
