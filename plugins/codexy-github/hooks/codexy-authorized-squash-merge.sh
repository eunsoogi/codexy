...
while [ "$#" -gt 0 ]; do
	...
	--body-file)
		...
		;;
	*)
		printf '%s\n' "unsupported authorized merge argument: $1" >&2
		exit 2
		;;
	esac
done
...
for required in "$expected_pr" "$message_file" "$repo" "$head_oid" "$subject" "$body_file"; do
	[ -n "$required" ] || {
		printf '%s\n' 'missing required authorized merge argument' >&2
		exit 2
	}
done
...
# shellcheck disable=SC2016 # GraphQL variables are expanded by GitHub, not this shell.
