...
def apply_remote_urls(
    config: str | None, remote_urls: tuple[tuple[str, str, str], ...]
) -> str | None:
...
def remote_config(
    ...
    try:
        for record in (item for item in result.stdout.split(b"\0") if item):
            ...
            if (
                ...
                return None
            remote = remotes.setdefault(
                match.group(1).casefold(), {"url": [], "pushurl": []}
            )
    ...
    for remote in remotes.values():
        targets.extend(
            ...
            if push
            else remote["url"] + remote["pushurl"]
        )
    return "".join(
