...
) -> tuple[list[str], ExecutionContext] | None:
    while args and (args[0].startswith("-") or assignment(args[0])):
        ...
        if assignment(option):
            ...
            try:
                ...
                return None
        elif option in {"-u", "--unset", "-C", "--chdir"} or (
            option.startswith(("-u", "-C")) and len(option) > 2
        ):
            attached = (
                option[:2] if len(option) > 2 and option[:2] in {"-u", "-C"} else option
            )
            value = (
                option[2:] if attached != option else args[1] if len(args) > 1 else None
            )
            if value is None:
                return None
            context = (
                ...
                else at(context, resolve_cwd(context.cwd, value))
            )
    ...
    while args and args[0].startswith("-"):
        ...
        matched = next(
            (
                ...
                if option == item
                or option.startswith(item + "=")
                or (len(item) == 2 and option.startswith(item) and len(option) > 2)
            ),
            None,
        )
        if matched is not None:
            value = option[len(matched) :].removeprefix("=") or (
        ...
        elif option in grammar.flags or any(
            option.startswith(item + "=")
            for item in grammar.flags
            if item.startswith("--")
        ):
            args = args[1:]
        else:
