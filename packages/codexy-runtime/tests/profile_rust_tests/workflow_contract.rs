...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    std::fs::write(&fixture.workflow, workflow.replacen("      - run: scripts/profile_rust_tests.py --shard", "      # cargo test --locked --all-targets stays inside the profiler\n      - run: scripts/profile_rust_tests.py --shard", 1))?;
