...
fn fixture_with(command: &str) -> Result<GateFixture, Box<dyn std::error::Error>> {
    ...
    std::fs::write(&fixture.workflow, workflow.replacen("      - run: scripts/profile_rust_tests.py --shard ${{ matrix.shard }} --receipt receipts/posix-${{ matrix.shard }}.json\n", &format!("      - run: scripts/profile_rust_tests.py --shard ${{ matrix.shard }} --receipt receipts/posix-${{ matrix.shard }}.json\n      - run: {command}\n"), 1))?;
    Ok(fixture)
}
