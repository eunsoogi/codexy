...
fn validator_rejects_false_clean_synced_pushed_child_handoff() -> TestResult {
    assert_rejects_child_handoff(
        ...
        r#"{"number":204,"state":"OPEN","isDraft":false,"mergeStateStatus":"CLEAN","reviewDecision":"APPROVED","headRefOid":"1111111111111111111111111111111111111111","latestReviews":[{"body":"Didn't find any major issues.\n\nReviewed commit: `1111111111111111111111111111111111111111`","author":{"login":"automated-review"},"submittedAt":"2026-07-03T00:00:00Z"}],"worktreeStatus":"M src/validation/manifest.rs\n?? tests/validator_roles.rs","reviewThreads":{"pageInfo":{"hasNextPage":false},"nodes":[]}}"#
        .to_owned(),
        "child handoff",
    )
}
...
fn validator_allows_child_handoff_with_matching_clean_evidence() -> TestResult {
    let output = validate_handoff_with_pr_state(
        "Child handoff: branch clean, synced, and pushed at 068dbb247b7755035223c91ee39f26830f3c1609. Packaged Codexy Sentinel Turing: PASS on current head 068dbb247b7755035223c91ee39f26830f3c1609. PR ready for parent handoff; parent will handle merge gates.\n",
        &pr_state_with(
...
fn pr_state_with(fields: &str) -> String {
    format!(
        r#"{{"number":204,"state":"OPEN","isDraft":false,"reviewDecision":"APPROVED","reviewProfile":"light","latestReviews":[{{"body":"Didn't find any major issues.\n\nReviewed commit: `068dbb247b7755035223c91ee39f26830f3c1609`","author":{{"login":"automated-review"}},"submittedAt":"2026-07-03T00:00:00Z"}}],{fields}}}"#
    )
}
