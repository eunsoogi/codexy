...
fn repository_only_skills_are_discoverable_without_plugin_exposure() -> TestResult {
    ...
    let documented = std::fs::read_to_string(root.join("docs/architecture/skill-boundaries.md"))?;
    let expected = PROJECT_SKILLS
        .iter()
    ...
    exact_names(
        &rows(&documented, "Repository-only skills")?,
        &expected,
        "repository skill",
        4,
    )?;
    ...
    for skill in PROJECT_SKILLS {
        ...
        assert!(!root.join("plugins/codexy/skills").join(skill).exists());
    }
    Ok(())
}
