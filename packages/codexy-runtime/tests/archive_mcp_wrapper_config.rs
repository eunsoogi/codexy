...
fn run_gate(archive: &std::path::Path, plugin_root: &std::path::Path) -> std::process::Output {
    Command::new(codexy_runtime::paths::repository_root().join("scripts/inspect-release-archive"))
        .arg(archive)
        .arg(plugin_root)
        .output()
...
fn entrypoints(config: &std::path::Path) -> Vec<String> {
    let output = Command::new(
        codexy_runtime::paths::repository_root().join("scripts/inspect-mcp-entrypoints"),
    )
    .arg(config)
    .output()
