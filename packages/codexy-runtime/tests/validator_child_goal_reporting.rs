...
Blocked goal user-decision gate: gate id=audit-375; blocker class=user-decision; decision owner=user; user question=Should the irreversible migration preserve identifiers?; user response=unanswered; decision branches=preserve identifiers|replace identifiers; material impact=the choice changes persisted identifiers and migration behavior; safe default=unavailable; in-scope action=unavailable
