...
Blocked goal user-decision gate: gate id=audit-375; blocker class=user-decision; decision owner=user; user question=Should the irreversible migration preserve identifiers?; user response=unanswered; decision branches=preserve existing persisted identifiers|replace identifiers during migration; material impact=the choice changes persisted identifiers and migration behavior; safe default=unavailable; in-scope action=unavailable
Parent goal pre-delivery: operation=update_goal(blocked); parent task=019f49da-d44c-7e41-afde-8b1f7c58efa0; delivery=confirmed; task surface=codex task/thread; issue=#375; plan step=await-parent; branch=codexy/375-parent-goal-transition-reporting; worktree=/worktree; HEAD=abc123; clean/index=clean; evidence=parent delivery; next action=block goal; parent direction version=direction-1; transition key=375:blocked:external-impasse
...
Blocked goal pre-mutation check: gate id=audit-375; pre-delivery parent direction version=direction-1; current parent direction version=direction-1; cancellation=absent
