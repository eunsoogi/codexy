...
fn transition(call: &str) -> String {
    let (key, result, audit) = if call.contains("blocked") {
        ("417:blocked:wait", "blocked", "Blocked goal audit: audit id=wait-417; first monotonic ms=1000; observed monotonic ms=61000; minimum interval ms=60000; observation ids=one|two|three; state fingerprints=one|two|three; producer state=none; safe action=unavailable; wake route=unavailable\n")
    } else { ("417:complete:wait", "complete", "") };
    let check = call.contains("blocked").then_some("Blocked goal pre-mutation check: audit id=wait-417; pre-delivery parent direction version=direction-1; current parent direction version=direction-1; cancellation=absent\n").unwrap_or_default();
