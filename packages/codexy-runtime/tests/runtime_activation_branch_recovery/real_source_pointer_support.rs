...
pub(super) fn selected_runtime_version(root: &Path) -> Result<String, Box<dyn std::error::Error>> {
...
pub(crate) fn assert_activated_source_pointer(
    root: &Path,
...
pub(crate) fn assert_result(output: Output, success: bool, case: &str) {
    assert_eq!(
...
pub(crate) fn restore_pre_activation_runtime_inputs(
    repo: &Path,
    revision: &str,
) -> Result<(), Box<dyn std::error::Error>> {
    for relative in [
