...
fn draft_release_flow_checks_payload_tag_and_assets_before_baseline() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (mode, success, status) in [("success", true, "201"), ("draft-no-tag", true, "201"), ("concurrent", true, "422"), ("401", false, "401"), ("422", false, "422"), ("500", false, "500")] {
        ...
        if success {
            ...
            if mode == "draft-no-tag" { assert!(events.contains("draft-without-tag")); }
    ...
    for mode in ["non-draft", "wrong-target", "duplicate", "stale-with-tag", "stale-assets", "stale-non-draft", "stale-prerelease", "stale-malformed-target", "stale-unrelated-target", "stale-main-moved", "stale-patch-failure", "stale-race-check-failure", "untagged-malformed", "untagged-alternate", "untagged-present-tag", "untagged-assets", "untagged-non-draft", "untagged-prerelease", "untagged-duplicate", "untagged-wrong-target", "untagged-list-failure", "untagged-patch-failure", "untagged-main-moved"] {
        let (output, events) = run_release_fixture(&publisher, mode)?;
        assert!(!output.status.success(), "mode={mode} was admitted");
        assert!(!events.contains("upload:") && !events.contains("baseline") && !events.contains("retarget"), "mode={mode} crossed a recovery boundary"); if mode == "stale-race-check-failure" { assert!(events.contains("tag-read-2")); } if mode.starts_with("untagged-") { assert!(!events.contains("release-create")); }
    }
    Ok(())
}
