...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    for literal in [
        ...
        assert!(dispatch.contains(literal), "missing path/dispatch literal: {literal}");
    }
    Ok(())
}
...
-> Result<(), Box<dyn std::error::Error>> {
    for name in [
        ...
        assert_ne!(document["permissions"]["actions"], "write", "{name} must not dispatch workflows");
        assert_eq!(document["on"]["workflow_dispatch"]["inputs"]["head_sha"]["required"], true, "{name}");
        ...
        for job in jobs.values() {
            for step in job["steps"].as_sequence().ok_or("steps")? {
                if step["uses"] == "actions/checkout@v7" {
                    ...
                    assert!(checkout.contains("inputs.head_sha"), "{name} checkout is not dispatch-bound");
                    checked_out += 1;
                }
            }
        }
        ...
        assert!(text.contains("EXPECTED_HEAD_SHA") || text.contains("HEAD_SHA"), "{name} lacks exact-head proof");
    }
    let touched = workflow("touched-loc-gate.yml")?;
    assert_eq!(touched["on"]["workflow_dispatch"]["inputs"]["base_sha"]["required"], true);
    let touched_run = run(&touched, "touched-loc", "Validate touched implementation LOC")?;
    support::assert_structured_literals(
        touched_run,
...
#[test]
fn rust_dispatch_has_an_explicit_non_measurement_ci_mode()
-> Result<(), Box<dyn std::error::Error>> {
