...
const CONNECTOR_HOOKS: &[(&str, &str)] = &[
    ...
    ("^(?:mcp__codex_apps__github_(create|update)_pull_request|github\\.(create|update)_pull_request)$", "codexy-repository-pull-request"),
...
#[test]
fn pull_request_registration_covers_canonical_and_normalized_connector_names() -> Result<(), Box<dyn std::error::Error>> {
    let matcher = regex::Regex::new(CONNECTOR_HOOKS[1].0)?;
    for (tool, matched) in [
        ("mcp__codex_apps__github_create_pull_request", true),
        ("mcp__codex_apps__github_update_pull_request", true),
        ("github.create_pull_request", true),
        ("github.update_pull_request", true),
        ("github.update_issue", false),
    ] {
        assert_eq!(matcher.is_match(tool), matched, "{tool}");
    }
    Ok(())
}
