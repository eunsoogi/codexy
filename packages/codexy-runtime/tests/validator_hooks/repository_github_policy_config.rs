...
const PR_MATCHER: &str = "^(?:mcp__codex_apps__github_(create|update)_pull_request|github\\.(create|update)_pull_request)$";
const MERGE_MATCHER: &str = "^mcp__codex_apps__github_(merge_pull_request|enable_auto_merge)$";
const NESTED_EXEC_MATCHER: &str = "^functions\\.exec$";
...
fn write_config(root: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let groups = [
        ...
        (PR_MATCHER, "codexy-repository-pull-request"),
        (MERGE_MATCHER, "codexy-repository-merge"),
        (NESTED_EXEC_MATCHER, "codexy-repository-github-exec"),
    ];
