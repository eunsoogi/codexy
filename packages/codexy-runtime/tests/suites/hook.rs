#[path = "../validator_github_labels_completion_waiting.rs"]
mod validator_github_labels_completion_waiting;

#[path = "../validator_github_labels_handoff.rs"]
mod validator_github_labels_handoff;

#[path = "../validator_github_labels_negative_readiness.rs"]
mod validator_github_labels_negative_readiness;

#[path = "../validator_github_labels_stacked_handoff.rs"]
mod validator_github_labels_stacked_handoff;

#[path = "../validator_hook_loc_contract.rs"]
mod validator_hook_loc_contract;

#[path = "../validator_hook_purpose_topology.rs"]
mod validator_hook_purpose_topology;

#[path = "../validator_hook_review_feedback.rs"]
mod validator_hook_review_feedback;

#[path = "../validator_hook_safety_tokens.rs"]
mod validator_hook_safety_tokens;

#[path = "../validator_hook_single_concern_topology.rs"]
mod validator_hook_single_concern_topology;

#[path = "../validator_hook_watcher_interrupt.rs"]
mod validator_hook_watcher_interrupt;

#[path = "../validator_github_hooks_topology.rs"]
mod validator_github_hooks_topology;

#[path = "../validator_github_title_hooks.rs"]
mod validator_github_title_hooks;

#[path = "../validator_child_thread_creation_hook.rs"]
mod validator_child_thread_creation_hook;

#[path = "../validator_subagent_ownership_hook.rs"]
mod validator_subagent_ownership_hook;

#[path = "../validator_subagent_ownership_delta.rs"]
mod validator_subagent_ownership_delta;

#[cfg(windows)]
#[path = "../validator_subagent_ownership_windows.rs"]
mod validator_subagent_ownership_windows;

#[path = "../validator_hooks.rs"]
mod validator_hooks;

#[path = "../validator_issue_title_hook.rs"]
mod validator_issue_title_hook;

#[path = "../validator_merge_message.rs"]
mod validator_merge_message;

#[path = "../validator_merge_message_conventional_subject.rs"]
mod validator_merge_message_conventional_subject;

#[path = "../validator_merge_message_pr_suffix.rs"]
mod validator_merge_message_pr_suffix;

#[path = "../validator_pr_title.rs"]
mod validator_pr_title;

#[path = "../validator_pr_workflow_docs.rs"]
mod validator_pr_workflow_docs;
