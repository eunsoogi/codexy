...
#[test]
fn validation_workflows_are_read_only_and_disable_checkout_credentials()
    ...
    for name in ["python-package.yml", "plugin-runtime-binaries.yml"] {
        ...
        for job in document["jobs"].as_mapping().ok_or("jobs")?.values() {
            if let Some(permissions) = job.get("permissions") {
                assert_exact(mapping(permissions)?, "contents", "read")?;
            }
            for step in job["steps"].as_sequence().ok_or("steps")? {
                if step["uses"].as_str() == Some("actions/checkout@v7") {
...
#[test]
fn staging_activation_and_final_release_write_only_at_explicit_boundaries()
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let attestation = publisher["jobs"]["verify-v1-3-0"]["steps"]
        ...
        .and_then(|steps| {
            steps.iter().find(|step| {
                step["name"] == "Verify public release attestations with a read-only token"
            })
        })
...
fn mapping(value: &Value) -> Result<&Mapping, Box<dyn std::error::Error>> {
    value.as_mapping().ok_or_else(|| "mapping".into())
}
fn assert_exact(
    mapping: &Mapping,
    name: &str,
    ...
    assert_eq!(mapping[Value::String(name.into())], value);
    Ok(())
}
...
fn command(run: &str, words: &[&str]) -> bool {
...
fn checkout_persists(value: &Value, job: &str) -> Result<bool, Box<dyn std::error::Error>> {
    value["jobs"][job]["steps"]
        .as_sequence()
        .and_then(|steps| {
            steps
                .iter()
                .find(|step| step["uses"] == "actions/checkout@v7")
        })
        .and_then(|step| step["with"]["persist-credentials"].as_bool())
        .ok_or_else(|| "checkout credentials".into())
}
