...
fn codegraph_search_reports_deterministic_metadata_for_ordinary_and_missing_results(
    ...
    let first_text = search_text(client, root.path(), "ENTRY", 10)?;
    let second_text = search_text(client, root.path(), "ENTRY", 10)?;
    ...
    let missing = search(client, root.path(), "MISSING", 10)?;
    ...
    let result_limited = search(client, root.path(), "ENTRY", 1)?;
...
fn codegraph_search_bounds_a_one_megabyte_line_without_emitting_it(
    ...
    let text = search_text(client, root.path(), "ENTRY_MEGABYTE", 1)?;
...
fn codegraph_search_keeps_a_late_match_inside_the_line_byte_limit(
    ...
    let result = search(client, root.path(), "NEEDLE", 1)?;
...
fn codegraph_search_honors_line_byte_boundaries_without_splitting_utf8(
    ...
    for (name, bytes) in [("below", MATCH_LIMIT_BYTES - 1), ("exact", MATCH_LIMIT_BYTES), ("above", MATCH_LIMIT_BYTES + 1)] {
        ...
        let result = search(client, root.path(), &format!("ENTRY_LINE_{name}"), 1)?;
    ...
    let result = search(client, root.path(), "ENTRY_UTF8", 1)?;
...
fn codegraph_search_honors_total_content_byte_boundaries(
    client: &mut McpClient,
) -> Result<(), Box<dyn std::error::Error>> {
    let base = tempfile::tempdir()?;
    write_total_fixture(base.path(), 0)?;
    let base_result = search(client, base.path(), "ENTRY_TOTAL", 10)?;
    ...
    for (name, adjustment, truncated) in [("below", -1_isize, false), ("exact", 0, false), ("above", 1, true)] {
        ...
        let result = search(client, root.path(), "ENTRY_TOTAL", 10)?;
        assert!(
