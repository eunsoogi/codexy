...
fn validator_cli_rejects_unresolved_pending_worktree_ids() -> TestResult {
    for handoff in [
        ...
        "pendingWorktreeId local:edge reached bounded timeout after list_threads searches by pending id. Branch codexy/291, PR #292, SHA abc123, review-thread id O8K31; safe retry/reassignment: yes.\n",
...
fn validator_cli_allows_resolved_pending_worktree_ids() -> TestResult {
    for handoff in [
        ...
        "pendingWorktreeId local:edge reached bounded timeout after list_threads searches by pending id, branch, PR, SHA, and review-thread id; duplicate owners: none; safe retry/reassignment: yes.\n",
...
fn validate_open_pr_handoff(handoff: &str) -> OutputResult {
    validate_handoff_with_pr_state(
        handoff,
        r#"{"number":128,"state":"OPEN","isDraft":false,"mergeStateStatus":"CLEAN","reviewDecision":"APPROVED","reviewProfile":"light","headRefOid":"32b03a210b3defb2d29dd352283ea2488e60d893","latestReviews":[{"body":"Didn't find any major issues.\n\nReviewed commit: `32b03a210b3defb2d29dd352283ea2488e60d893`","author":{"login":"automated-review"},"submittedAt":"2026-06-22T12:50:03Z"}],"reviewThreads":{"pageInfo":{"hasNextPage":false},"nodes":[]}}"#,
    )
}
