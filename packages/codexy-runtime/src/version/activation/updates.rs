...
pub(super) fn prepare(
    ...
    super::super::require_semver(bootstrap_version)?;
    let core_aware = validate_core_aware_tree(repo_root)?;
    if bootstrap_version != super::super::bootstrap::CANDIDATE_VERSION {
        bail!(
            "bootstrap version must be verified public candidate {}",
            super::super::bootstrap::CANDIDATE_VERSION
        );
    }
...
fn publish_contract_update(root: &Path, version: &str, release_tag: &str) -> Result<Update> {
    ...
    let selected_bootstrap_matches = contract["bootstrap"]["selectedVersion"]
        .as_str()
        .is_some_and(|selected| selected == super::super::bootstrap::VERSION);
...
fn runtime_release_update(root: &Path, release: &Value) -> Result<Update> {
    Ok(Update {
        path: root.join("plugins/codexy-devtools/runtime-release.json"),
        bytes: format!(
            "{}\n",
            serde_json::to_string_pretty(&canonical(release.clone()))?
        )
        .into_bytes(),
        delete: false,
    })
...
fn bootstrap_update(root: &Path, version: &str) -> Result<Update> {
    let path = super::super::runtime_package_path(root, "src/version/bootstrap.rs");
    let source = fs::read_to_string(&path)
        .with_context(|| format!("reading selected bootstrap metadata: {}", path.display()))?;
    let previous = format!(
        "pub(super) const VERSION: &str = \"{}\";",
        super::super::bootstrap::VERSION
    );
...
pub(super) fn apply_with<F>(updates: &[Update], apply: F) -> Result<()>
where
