...
fn cargo_manifest_path(root: &Path) -> PathBuf {
    super::runtime_package_path(root, "Cargo.toml")
}

fn cargo_lock_path(root: &Path) -> PathBuf {
    super::runtime_package_path(root, "Cargo.lock")
}

pub(super) fn check_version(root: &Path, manifest_version: &str) -> Result<()> {
    let cargo_path = cargo_manifest_path(root);
    ...
    let cargo_lock_path = cargo_lock_path(root);
