...
const NATIVE_ADMISSION_FORBIDDEN_SCRIPT_FRAGMENTS: &[&str] = &[
    "~/", "$HOME/", "$HOME", "${HOME}/", "${HOME}", "/Users/", "/home/", "~/.codex",
    "$HOME/.codex", "${HOME}/.codex", ".codex/", ".git/", "auth.json", "history.jsonl",
    "PLUGIN_DATA", "CLAUDE_PLUGIN_DATA", "npm", "curl", "codex plugin", "codex mcp", ">",
];
...
pub(super) fn check_script(path: &Path, event: &str, script_path: &Path) -> Result<()> {
    ...
    let forbidden = if native_admission { NATIVE_ADMISSION_FORBIDDEN_SCRIPT_FRAGMENTS } else { FORBIDDEN_SCRIPT_FRAGMENTS };
