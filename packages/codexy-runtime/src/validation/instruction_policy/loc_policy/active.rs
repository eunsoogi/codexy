pub(super) fn governs_loc_exception(words: &[String], must: usize) -> bool {
    is_direct_exception_permission(words.get(must + 1).map(String::as_str))
...
fn is_direct_exception_permission(word: Option<&str>) -> bool {
    matches!(word, Some("use")) || is_permission(word)
}
