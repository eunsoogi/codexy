...
pub fn tools() -> Vec<ToolDef> {
    vec![
        ...
        ToolDef::new(
            "wait_watcher",
            "Wait for bounded material Watcher reports from a durable cross-process queue for up to 60 minutes; host cancellation releases the wait immediately. If the session TTL expires, returns status=expired with empty events, the unchanged nextCursor, and the durable queue preserved for later reads.",
            json!({
                "type":"object", "additionalProperties":false,
