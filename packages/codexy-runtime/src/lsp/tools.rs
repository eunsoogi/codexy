...
use crate::lsp::pathing::{
    ...
    resolve_path, workspace_root_from_args,
};
    ...
    match name {
        ...
        "lsp_for_path" => {
            ...
            text_json(&matching_servers(
                ...
                path_resolution_root_from_args(args).map(|root| root.0),
            )?)
        }
...
fn status_result(args: &Value) -> Result<Value> {
    ...
    let path_root = path_resolution_root_from_args(args);
    let file_path = resolve_path(raw_path, path_root.map(|root| root.0))?;
    let server = select_server(args, &file_path, path_root.map(|root| root.0))?;
    text_json(&json!({
        "path": file_path,
...
fn operation_result(args: &Value, method: LspMethod) -> Result<Value> {
    ...
    let workspace_root = workspace_root_from_args(args, &file_path)?;
