...
pub(super) struct PathResolutionRoot<'a>(pub(super) &'a str);
...
pub(super) fn path_resolution_root_from_args(args: &Value) -> Option<PathResolutionRoot<'_>> {
    args.get("root")
        .or_else(|| args.get("workspaceRoot"))
        .and_then(Value::as_str)
        .map(PathResolutionRoot)
}

pub(super) fn match_path_from_args(path: &str, args: &Value) -> Result<String> {
    path_resolution_root_from_args(args).map_or_else(
        || Ok(path.to_owned()),
        |root| resolve_path(path, Some(root.0)),
    )
}

pub(super) fn workspace_root_from_args(args: &Value, file_path: &str) -> Result<WorkspaceRoot> {
    if let Some(root) = args.get("workspaceRoot").and_then(Value::as_str) {
        return Ok(WorkspaceRoot(resolve_root(root)?));
    }
    Ok(WorkspaceRoot(workspace_root_for_file(file_path)))
}
