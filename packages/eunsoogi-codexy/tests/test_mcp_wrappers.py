...
class McpWrapperTests(unittest.TestCase):
    ...
    def test_relative_home_never_executes_cached_runtime(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for server in ("lsp", "codegraph"):
                key = runtime_cache_key(manifest=manifest, package_override=False, identity=["https://github.com/eunsoogi/codexy", "", "linux-x86_64", "stdio-newline-v1", "package-default\n", f"codexy-mcp-{server}"]); cached = root / ".cache" / "codexy" / "runtime" / key / "bin" / f"codexy-mcp-{server}"
                cached.parent.mkdir(parents=True)
                cached.write_text('#!/bin/sh\necho cached-runtime\n')
                cached.chmod(0o755)
                (cached.parents[1] / "plugin.json").write_text(json.dumps({"version": "1.2.1"}))
                completed = subprocess.run([self.wrapper(server)], cwd=root, env={"PATH": f"{bin_dir}:/usr/bin:/bin", "HOME": ".", "UV_OFFLINE": "1"}, capture_output=True, text=True)
                self.assertEqual(completed.returncode, 127)
