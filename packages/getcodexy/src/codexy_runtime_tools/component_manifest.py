...
def load_component_manifest() -> ComponentManifest:
    return parse_component_manifest(
...
def valid_semver(value: object) -> bool:
    return (
        ...
        and all(
            len(component) < len(str(MAX_SEMVER_COMPONENT))
            or len(component) == len(str(MAX_SEMVER_COMPONENT))
            and component <= str(MAX_SEMVER_COMPONENT)
...
def _parse_manifest(data: object) -> ComponentManifest:
    ...
    if not isinstance(data["components"], list) or not isinstance(
        data["compatibleCombinations"], list
    ):
    ...
    ids, versions = (
        ...
        {component.version for component in components},
    )
    ...
    if (
        len({component.plugin for component in components}) != len(components)
        or len({component.asset.plugin_id for component in components})
        != len(components)
        or len({component.asset.package_root for component in components})
        != len(components)
    ):
        raise ValueError("component manifest assets must uniquely identify components")
    if any(
        dependency not in ids
        or dependency == component.id
        or component.dependencies.count(dependency) > 1
    ...
    compatible = tuple(
        _combination(item, ids, version) for item in data["compatibleCombinations"]
    )
    ...
    return ComponentManifest(
        marketplace, version, components, compatible, domain_errors
    )
...
def _marketplace(value: object) -> Marketplace:
    if (
        ...
        or set(value) != {"name", "source"}
        or not all(isinstance(value[key], str) and value[key] for key in value)
        or value["name"] != MARKETPLACE
        or value["source"] != OFFICIAL
    ):
...
def _component(value: object, marketplace: Marketplace) -> Component:
    ...
    if any(
        ...
        for key in ("id", "plugin", "version")
    ) or not valid_semver(value["version"]):
        raise ValueError("component manifest component has invalid text")
    return Component(
        value["id"],
        value["plugin"],
        value["version"],
        _strings(value["dependencies"], "dependencies"),
        _asset(value["asset"], value["plugin"], marketplace),
    )
...
def _asset(value: object, plugin: str, marketplace: Marketplace) -> Asset:
    ...
    root, paths = (
        value.get("packageRoot"),
        _strings(value.get("requiredPaths"), "requiredPaths", nonempty=True),
    )
    if (
        value.get("pluginId") != f"{plugin}@{marketplace.name}"
        or root != f"plugins/{plugin}"
        or len(paths) != len(set(paths))
        or any(path.startswith("/") or ".." in path.split("/") for path in paths)
    ):
...
def _combination(value: object, ids: tuple[str, ...], version: str) -> tuple[str, ...]:
    ...
    if value.get("version") != version or components != tuple(
        item for item in ids if item in components
    ):
