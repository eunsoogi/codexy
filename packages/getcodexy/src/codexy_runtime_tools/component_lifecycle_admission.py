...
from .component_resolver import (
    admit_bootstrap_recovery_inventory,
...
def admitted_bootstrap_recovery_selection(
    ...
    return admit_bootstrap_recovery_inventory(
        manifest, inventory, marketplace_root, before, target
    )
...
def replay_receipt(
    ...
    if (
        receipt["operation_id"] == identifier
        and receipt["command"] == command
        and receipt["requested_components"] == list(requested)
    ):
        return receipt
    raise ValueError(
        f"operation receipt conflicts with requested operation: {identifier}"
    )
...
def admit_pending_receipt(
    ...
    _validate_receipt(manifest, receipt)
    if (
        receipt["operation_id"] != journal.identifier
        or receipt["command"] != journal.command
        or receipt["requested_components"] != list(journal.requested)
        or receipt["resolved_components"] != list(journal.resolved)
        or receipt["selection_before"] != list(journal.before)
    ):
        raise ValueError(
            f"pending transaction receipt conflicts with journal: {journal.identifier}"
        )
    if (
    ...
    if (
        ...
        and receipt["outcome"] == "rolled-back"
        and receipt["selection_after"] == list(journal.before)
    ):
        return receipt
    raise ValueError(
        f"pending transaction receipt conflicts with journal: {journal.identifier}"
