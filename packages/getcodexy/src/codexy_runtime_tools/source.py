...
class ExplicitRuntimeSource:
    ...
    def select(
        ...
        *,
        ...
        if len(sources) != 1:
            raise ValueError(
                "explicit runtime package requires exactly one non-empty source"
            )
        if len(package_sha256) != 64 or any(
            character not in "0123456789abcdef" for character in package_sha256
        ):
            raise ValueError(
                "explicit runtime package source requires a lowercase SHA-256"
            )
...
class RuntimeSourceIdentity:
    ...
    def create(
        cls,
        *,
        explicit: ExplicitRuntimeSource | None,
        package_sha256: str,
        package_url: str,
        ...
        if release:
            return cls(
                RuntimeSourceMode.SELECTED_RELEASE,
                release.artifact.sha256,
                {"tag": release.artifact.tag, "url": release.artifact.url},
                release,
            )
        return cls(
            RuntimeSourceMode.LEGACY_DEFAULT,
            package_sha256,
    ...
    def git_fallback(cls, *, repository: str, commit: str) -> "RuntimeSourceIdentity":
        ...
        return cls(
            RuntimeSourceMode.GIT_FALLBACK,
            None,
    ...
    def cache_key(self, *, platform: str, server: str) -> str | None:
        ...
        if self.mode is RuntimeSourceMode.LEGACY_DEFAULT:
            return None
        return (
            "v3-" + hashlib.sha256(self._encoded_identity(platform, server)).hexdigest()
        )
    ...
    def marker(
        ...
        if self.mode is RuntimeSourceMode.SELECTED_RELEASE:
            assert self.release is not None
            return self.release.marker(
                platform=platform, server=server, binary_sha256=binary_sha256
            )
        if self.mode is RuntimeSourceMode.LEGACY_DEFAULT:
            return None
        if self.mode is RuntimeSourceMode.GIT_FALLBACK:
            return {
                "schema": "codexy-runtime-git-marker/v1",
                "identity": self._identity(platform, server),
                "installedBinarySha256": binary_sha256,
            }
        return {
            "schema": "codexy-runtime-override-marker/v1",
            "identity": self._identity(platform, server),
            "installedBinarySha256": binary_sha256,
        }
    ...
    def _encoded_identity(self, platform: str, server: str) -> bytes:
        return json.dumps(
            self._identity(platform, server), sort_keys=True, separators=(",", ":")
        ).encode()
