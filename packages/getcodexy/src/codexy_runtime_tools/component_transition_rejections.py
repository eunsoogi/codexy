...
class Rejection:
    ...
    @classmethod
    def from_failure(cls, stage: RejectionStage, failure: ComponentResolutionError | StateFailure) -> "Rejection":
        code = failure.code if isinstance(failure, ComponentResolutionError) else failure.value
        try:
            return cls(stage, RejectionKind(code), str(failure))
        except ValueError as error:
            raise ValueError("component transition has an unreachable rejection") from error

    def validate(self, manifest: ComponentManifest, command: Command, requested: tuple[str, ...], before: tuple[str, ...], planner: Planner) -> None:
        if not any((variant.stage, variant.kind) == (self.stage, self.kind) for variant in variants(manifest, command, requested, before, planner)):
            raise ValueError("component transition rejection has an invalid stage")


def valid_rejection(manifest: ComponentManifest, command: Command, requested: tuple[str, ...], before: tuple[str, ...], errors: tuple[str, ...], planner: Planner) -> bool:
    if len(errors) != 1:
        return False
    try:
        ...
        return False
    return any(variant.kind is kind for variant in variants(manifest, command, requested, before, planner))


def variants(manifest: ComponentManifest, command: Command, requested: tuple[str, ...], before: tuple[str, ...], planner: Planner) -> frozenset[Rejection]:
    request = _request_rejection(manifest, command, requested)
    if request is not None:
        return frozenset({Rejection(RejectionStage.REQUEST, request)}) if not before else frozenset()
    result = {Rejection(RejectionStage.PRESTATE, RejectionKind.INCONSISTENT_INSTALLED_STATE)}
    if not before:
...
_HOST_KINDS = (
    RejectionKind.INVALID_INSTALLED_INVENTORY, RejectionKind.CONFLICTING_INSTALLED_STATE,
    RejectionKind.UNKNOWN_INSTALLED_COMPONENT, RejectionKind.MIXED_VERSION_STATE,
    RejectionKind.COMPONENT_VERSION_MISMATCH, RejectionKind.INCONSISTENT_INSTALLED_STATE,
)
