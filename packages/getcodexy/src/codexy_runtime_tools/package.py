...
RUNTIME_PLUGIN_ROOTS_FOLDED = frozenset(
    root.casefold() for root in RUNTIME_PLUGIN_ROOTS
)


class _GithubRedirectHandler(urllib.request.HTTPRedirectHandler):
    def redirect_request(
        self, request, file_pointer, status, message, headers, new_url
    ):
...
def _download(url: str, destination: Path, token: str = "") -> None:
    ...
    open_request = (
        urllib.request.build_opener(_GithubRedirectHandler()).open
        if token
...
def _artifact_package(api_url: str, work: Path) -> Path:
    ...
    selected = next(
        (
            ...
            and item["workflow_run"].get("head_repository_id")
    ...
    if token and not (
        _trusted_github_api(download_url)
        or _origin(download_url) == ("https", "github.com", None)
    ):
        ...
        with tarfile.open(archive, "r:gz") as package:
            ...
            for member in package.getmembers():
                ...
                if root.rstrip(". ") != root:
                    raise RuntimeError("runtime package has non-canonical plugin root")
                if (
                    root.casefold() in RUNTIME_PLUGIN_ROOTS_FOLDED
                    and root not in RUNTIME_PLUGIN_ROOTS
                ):
                    raise RuntimeError("runtime package has non-canonical plugin root")
...
def unpack_runtime(
    *,
    ...
    if plugin_root not in roots:
        raise RuntimeError("runtime package is missing the selected plugin root")
    if roots != {plugin_root} and not allow_mixed_plugin_roots:
        raise RuntimeError("runtime package contains mixed plugin roots")
    ...
    if (
        ...
        raise RuntimeError(
            "runtime package is missing its exact runtime binary or plugin manifest"
        )
