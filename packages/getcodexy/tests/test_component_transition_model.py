...
from codexy_runtime_tools.component_transition_rejections import (
    Rejection,
    RejectionStage,
    variants as rejection_variants,
)
from component_receipt_identity_cases import ComponentReceiptIdentityCases
...
class TransitionModelTests(ComponentReceiptIdentityCases, unittest.TestCase):
                    ...
                    with self.subTest(
                        ...
                        try:
                            plan = plan_transition(
                                manifest, command, requested, before, recorded
                            )
                        except ComponentResolutionError:
                            continue
                        snapshot = InventorySnapshot(
                            None if recorded is None else _inventory(before)
                        )
                        journal = Journal.decode(
                            plan.journal(
                                _identifier(command, before, requested, recorded),
                                snapshot,
                            ).encode()
                        )
                        ...
                        for outcome, after in (
                            ...
                            ("rolled-back", plan.before),
                        ):
                            receipt = OperationReceipt.decode(
                                journal.receipt(outcome, after).encode()
                            )
        ...
        for before in manifest.compatible_combinations:
            for command, requested in _requests():
                for variant in rejection_variants(
                    manifest, command, requested, before, plan_transition
                ):
                    with self.subTest(
                        before=before,
                        command=command,
                        ...
                        OperationReceipt.rejected(
                            _identifier(command, before, requested, None),
                            command,
                            requested,
                            before,
                            variant,
    ...
    def test_decoded_receipts_reject_single_field_mutations_and_unknown_values(
        self,
    ) -> None:
        manifest = load_component_manifest()
        plan = plan_transition(manifest, "install", ("github",), ("core",), ("core",))
        receipt = (
            plan.journal("op-mutation", InventorySnapshot(_inventory(("core",))))
            .receipt("completed")
            .encode()
        )
        mutations = {
    ...
    def test_rejected_receipts_accept_only_reachable_stage_variants(self) -> None:
        ...
        valid_host = OperationReceipt.rejected(
            ...
            Rejection.from_failure(
                RejectionStage.HOST,
                ComponentResolutionError("invalid-installed-inventory"),
            ),
        )
        ...
        invalid = (
            ...
            OperationReceipt.rejected(
                "op-request-after-host",
                "install",
                ("unknown",),
                ("core",),
                Rejection.from_failure(
                    RejectionStage.REQUEST,
                    ComponentResolutionError("unknown-component"),
                ),
            ),
            OperationReceipt(
                "op-pending",
        ...
        with self.assertRaisesRegex(ValueError, "stage"):
            Rejection.from_failure(
                RejectionStage.HOST, ComponentResolutionError("unknown-component")
            ).validate(manifest, "install", ("unknown",), (), plan_transition)
...
def _requests() -> tuple[tuple[str, tuple[str, ...]], ...]:
    ...
    return tuple(
        (command, selected)
        for command in ("install", "update", "remove", "bootstrap")
