    ...
    def test_fallback_rejects_noncanonical_plugin_records(self) -> None:
        ...
        with fixture({"core"}) as state:
            ...
            cases = {
                ...
                "foreign": {"marketplaceSource": {"sourceType": "git", "source": "https://example.invalid/foreign.git"}},
            }
            for name, changes in cases.items():
                with self.subTest(name=name):
                    candidate = deepcopy(record)
                    candidate.update(changes)
                    self.assertEqual(observe_installed_inventory(manifest, {"installed": [candidate]}).error, "conflicting-installed-state")
