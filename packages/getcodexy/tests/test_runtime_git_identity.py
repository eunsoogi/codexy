...
class RuntimeGitIdentityTests(unittest.TestCase):
    def test_git_fallback_has_an_exact_independent_cache_identity_and_marker(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source_identity_tests.RuntimeSourceIdentityTests._write_selected_release(
                root
            )
            cache = root / "cache"
            ...
            def install_git(
                _config: object, install_root: Path, installed: Path
            ) -> None:
            ...
            with (
                ...
                mock.patch.object(
                    runtime,
                    "install_package",
                    side_effect=RuntimeError("missing public package"),
                ),
            ...
            cases = (
                ...
                (
                    ...
                    replace(config, git_repository="https://example.test/other"),
                ),
                (
                    "different-platform",
                    replace(
                ...
                (
                    ...
                    replace(
                        config,
                        server="codegraph",
                        runtime_name="codexy-mcp-codegraph-linux-x86_64.bin",
                    ),
                ),
            )
            ...
            for corruption in ("missing", "corrupt", "package-marker"):
                if corruption == "missing":
                    ...
                    marker.write_text(
                        json.dumps(
                            base.source_identity.marker(
                                platform=config.platform,
                                server=config.server,
                                binary_sha256=hashlib.sha256(
                                    b"pinned Git runtime"
                                ).hexdigest(),
                            )
                        ),
                        encoding="utf-8",
                ...
                with (
                    self.subTest(marker=corruption),
                    mock.patch.dict(os.environ, environment, clear=True),
                    mock.patch.object(runtime, "_execute") as execute,
                    self.assertRaisesRegex(SystemExit, "127"),
                ):
    ...
    def test_failed_git_fallback_never_admits_partial_files(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            cache = root / "cache"
            config = replace(
                runtime.Configuration.load("lsp", root, []), git_fallback=True
            )

            def partial_install(
                _config: object, _install_root: Path, installed: Path
            ) -> None:
                installed.parent.mkdir(parents=True)
