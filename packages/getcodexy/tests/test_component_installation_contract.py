...
class ComponentInstallationContractTests(unittest.TestCase):
    def test_public_contract_freezes_component_dependencies_and_cli_semantics(
        self,
    ) -> None:
    ...
    def test_contract_fixtures_cover_happy_risky_regression_and_external_paths(
        self,
    ) -> None:
        fixtures = {
            fixture["id"]: fixture
            for fixture in json.loads(FIXTURES_PATH.read_text(encoding="utf-8"))[
                "fixtures"
            ]
        }

        self.assertEqual(
            fixtures["install-default"]["selection_after"],
            ["core", "github", "devtools"],
        )
        self.assertEqual(
            fixtures["install-github"]["selection_after"], ["core", "github"]
        )
        self.assertEqual(
            fixtures["update-preserves-selection"]["selection_after"],
            ["core", "devtools"],
        )
        transitions = {
            transition["id"]: transition
            for transition in json.loads(FIXTURES_PATH.read_text(encoding="utf-8"))[
                "state_transitions"
            ]
        }
        self.assertEqual(
        ...
        self.assertEqual(
            fixtures["remove-core-with-dependent"]["error"]["code"],
            "dependency-protected-removal",
        )
        self.assertEqual(
            fixtures["rollback-after-operation-failure"]["outcome"], "rolled-back"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            fixtures["status-json"]["stdout"]["schema"], "getcodexy.status.v1"
        )
        self.assertEqual(
        ...
        for transition in contract["state_transitions"]:
            with self.subTest(transition=transition["id"]):
                ...
                if transition["outcome"] == "rejected":
                    self.assertEqual(
                        transition["selection_before"], transition["selection_after"]
                    )
