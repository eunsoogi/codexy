...
from codexy_runtime_tools.component_transaction_state import clear_journal, read_journal, write_journal
...
class LifecycleFinalizationTests(unittest.TestCase):
    def test_failed_committed_journal_write_recovers_without_a_false_receipt(self) -> None:
        with fixture() as state:
            ...
            with patch.object(lifecycle, "write_journal", side_effect=reject_commit), self.assertRaisesRegex(OSError, "committed journal"):
                run_operation("install", ("core",), state.home, state.codex, state.run, operation_id="op-commit-write")
            receipt = inventory_path(state.home).parent / "receipts" / "op-commit-write.json"
            self.assertFalse(receipt.exists())
            self.assertEqual(read_journal(state.home).phase, "started")

            next_receipt = run_operation("install", ("devtools",), state.home, state.codex, state.run, operation_id="op-after-commit-write")
    ...
    def test_failed_cleanup_keeps_committed_state_for_idempotent_recovery(self) -> None:
        with fixture() as state:
            ...
            with patch.object(lifecycle, "clear_journal", side_effect=OSError("cleanup failed")), self.assertRaisesRegex(OSError, "cleanup failed"):
                run_operation("install", ("core",), state.home, state.codex, state.run, operation_id="op-cleanup")
    ...
    def test_receipt_collision_after_commit_recovers_on_the_next_invocation(self) -> None:
        with fixture() as state:
            with patch("codexy_runtime_tools.component_lifecycle.write_receipt", side_effect=ValueError("operation receipt already exists")), self.assertRaisesRegex(ValueError, "receipt already exists"):
                run_operation("install", ("core",), state.home, state.codex, state.run, operation_id="op-receipt-collision")
            receipt = inventory_path(state.home).parent / "receipts" / "op-receipt-collision.json"
            self.assertFalse(receipt.exists())
            self.assertEqual(read_journal(state.home).phase, "committed")
    ...
    def test_journal_deletion_syncs_its_parent_directory(self) -> None:
        with fixture() as state:
            from codexy_runtime_tools.component_transaction_state import InventorySnapshot, Journal

            journal = Journal("op-delete-sync", "install", ("core",), ("core",), (), ("core",), InventorySnapshot.capture(state.home), "started")
            write_journal(state.home, journal)
            with patch("codexy_runtime_tools.component_transaction_state.sync_parent_directory") as synced:
