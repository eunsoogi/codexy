...
from runtime_git_install_cases import RuntimeGitInstallCases


class RuntimeCliTests(RuntimeGitInstallCases, unittest.TestCase):
    ...
    def test_github_token_policy_uses_environment_then_cli_on_trusted_api(self) -> None:
        ...
        with (
            mock.patch.dict(
                "os.environ", {"GH_TOKEN": "environment-token"}, clear=True
            ),
        ...
        with (
            ...
            mock.patch.object(package.subprocess, "run", return_value=completed),
        ):
        ...
        with (
            ...
            self.assertEqual(
                _github_token_for("https://objects.example.test/artifact.zip"), ""
            )
        run.assert_not_called()
    ...
    def test_tokenless_download_uses_standard_urlopen(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                mock.patch(
                    "codexy_runtime_tools.package.urllib.request.urlopen",
                    return_value=response,
                ) as urlopen,
                mock.patch(
                    "codexy_runtime_tools.package.urllib.request.build_opener"
                ) as build_opener,
            ):
                package._download(
                    "https://downloads.example.test/package.tar.gz", destination
                )
            build_opener.assert_not_called()
            request = urlopen.call_args.args[0]
            self.assertEqual(
                request.full_url, "https://downloads.example.test/package.tar.gz"
            )
