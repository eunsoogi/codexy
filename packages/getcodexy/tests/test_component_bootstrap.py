...
class BootstrapTests(unittest.TestCase):
    def test_typed_idempotent_recovery_from_absent_record(self) -> None:
        with fixture({"core"}) as state:
            ...
            repeated = run_operation(
                "bootstrap",
                (),
                state.home,
                state.codex,
                state.run,
                operation_id="op-bootstrap-repeat",
            )
    ...
    def test_started_stale_bootstrap_recovers_an_intermediate_selection_before_a_different_install(
        self,
    ) -> None:
        with fixture(
            ...
            receipt = run_operation(
                ...
                state.codex,
                state.run,
                operation_id="op-after-stale-bootstrap",
            )
    ...
    def test_started_bootstrap_rejects_live_state_missing_a_journal_before_component(
        self,
    ) -> None:
        with fixture({"core"}, versions={"core": "1.2.0"}) as state:
            ...
            receipt = run_operation(
                ...
                state.codex,
                state.run,
                operation_id="op-after-missing-before",
            )
    ...
    def test_operands_have_a_typed_rejection(self) -> None:
        with fixture() as state:
            receipt = run_operation(
                ...
                state.codex,
                state.run,
                operation_id="op-bootstrap-operand",
            )
    ...
    def test_recovery_preserves_distinct_live_and_durable_prestate(self) -> None:
        with fixture({"core"}) as state:
            ...
            (state.home / "getcodexy" / "installed-components.json").write_text(
                '{"schema":"getcodexy.installed-component-inventory.v1","components":["core","github"]}'
            )
            receipt = run_operation(
                "bootstrap",
                (),
                ...
                state.codex,
                state.run,
                operation_id="op-bootstrap-mismatch",
            )
        self.assertEqual(receipt["selection_before"], ["core"])
        self.assertEqual(receipt["selection_after"], ["core", "github", "devtools"])
    ...
    def test_corrupt_bootstrap_durable_snapshot_fails_closed(self) -> None:
        with fixture({"core"}) as state:
            ...
            write_journal(
                state.home,
                Journal(
                    "op-bootstrap-corrupt",
                    "bootstrap",
                    (),
                    ("core", "github", "devtools"),
                    ("core",),
                    ("core", "github", "devtools"),
                    InventorySnapshot(invalid),
                    "started",
                ),
            )
