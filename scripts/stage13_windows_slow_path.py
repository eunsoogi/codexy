    ...
    class TimestampRuntimeTelemetry(base_type):
        ...
        def _record_line(self, line):
            ...
            if "Running " in line:
                ...
                self._target_boundaries.append({"target": self._current_target, "state": "started", "seconds": moment})
                ...
                self._pending_test = None
                self._target_boundaries.append({"target": self._current_target, "state": "completed", "seconds": moment})
        ...
        def _record_completion(self, name, outcome, moment):
            ...
            self._test_completions.append({"test": f"{self._current_target}::{name.removesuffix(' - should panic')}", "outcome": outcome, "seconds": moment, "gap_seconds": gap})
        ...
        def finish(self):
            ...
            receipt = {
                ...
                "perturbation_upper_bound_percent": round(100 * upper_bound / elapsed, 9) if elapsed else 0.0,
...
def run(trace_path: Path, profile_arguments: list[str]) -> int:
    ...
    profile.RuntimeTelemetry = instrument_runtime(profile.RuntimeTelemetry, trace_path, hashlib.sha256(PROFILE.read_bytes()).hexdigest())
    previous = sys.argv
    try:
