...
def producer(
    job: dict[str, object], runner: str, timeout: str, command: str, receipt: str
) -> bool:
    ...
    if (
        ...
        or job.get("runs-on") != runner
        or job.get("timeout-minutes") != timeout
        or job.get("if") is not None
    ):
        return False
    setup = (
        WINDOWS_SETUP
        if runner.startswith("windows")
        else ({"run": "sudo apt-get update && sudo apt-get install --yes ripgrep"},)
    )
...
def aggregate(job: dict[str, object]) -> bool:
    if (
        set(job) != {"needs", "if", "runs-on", "timeout-minutes", "steps"}
        or job.get("needs") != "[rust-test, windows-rust-test]"
        or job.get("if") != "always()"
        or job.get("runs-on") != "ubuntu-latest"
        or job.get("timeout-minutes") != "6"
    ):
        return False
    steps = job.get("steps")
...
def enforce_shard_workflow(
    jobs: dict[str, list[str]], context: Callable[[list[str]], dict[str, object]]
) -> bool:
    if set(jobs) != JOBS:
        return False
    expected = (
    ...
    if any(
        "cache" in "\n".join(jobs[name]).casefold()
        or "retry" in "\n".join(jobs[name]).casefold()
        or "continue-on-error" in "\n".join(jobs[name]).casefold()
        for name in jobs
    ):
        return False
    return all(
        producer(contexts[name], runner, timeout, command, receipt)
        for name, runner, timeout, command, receipt in expected
    ) and aggregate(contexts["rust-test-aggregate"])
