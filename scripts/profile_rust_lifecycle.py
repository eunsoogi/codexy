...
from profile_rust_accounting import (
    ...
    linux_cargo_descendants_snapshot,
)
...
def run_workload(
    ...
    with tempfile.TemporaryDirectory(prefix="codexy-profile-") as directory:
        ...
        with receipt_environment(Path(directory)) as (receipt_dir, environment):
            metrics_path, command_metrics_path = configure_metrics(
                environment, Path(directory)
            )
            temporary_root = (
                isolated_windows_test_root(environment)
                if windows
                else nullcontext(None)
            )
            with (
                temporary_root as temp_root,
                capture_path.open("wb", buffering=0) as capture,
            ):
                if temp_root:
                ...
                if job is None:
                    ...
                    process = launch_windows_workload(
                        job, root, capture, workload, environment=environment
                    )
                targets = (
                    declared_targets
                    if declared_targets is not None
                    else declared_test_targets(root)
                    if root is not None and (root / "Cargo.toml").is_file()
                    else ()
                )
        ...
        phases["fixture-telemetry-json"] = telemetry(
            ...
            environment,
            metrics_path,
            temp_root.telemetry() if temp_root else None,
        )
        ...
        if temp_root is not None:
            phases["windows-temp-cleanup-receipt-json"] = json.dumps(
                temp_root.telemetry(), sort_keys=True
            )
