...
remote_ref() {
	...
	lines="$(git -C "$PROOF_REPO" ls-remote "$REMOTE" "$ref" 2>/dev/null)" || die "unable to read remote ref"
...
[[ "$(remote_ref "refs/heads/$EXPECTED_PR_BASE_BRANCH")" == "$EXPECTED_PR_BASE_SHA" ]] || die "pull request base branch is stale or foreign"
[[ "$(remote_ref "refs/heads/$HEAD_REF")" == "$HEAD_SHA" ]] || die "proof head branch is stale"
...
[[ ! -e "$CODEX_HOME/auth.json" && ! -L "$CODEX_HOME/auth.json" ]] || die "Codex home contains inherited or generated auth state"
...
specs={"artifact-refresh":("AR",["schema","artifact","governing_source","outcome","removed","proof_handle","handoff_reason"],"codexy.artifact-refresh.v1"),"blind-read":("BR",["interpreted_purpose","unresolved_reference","action_blocker"],None),"project-brief":("PB",["objective","owner","verified_phase","changes_since_touch","decision_required","evidence_handle","next_action","done_when"],"codexy.project-brief.corpus.v1")}; contracts={name:(root/"plugins/codexy/skills"/name/"contract.md").read_text(encoding="utf-8") for name in specs}
for name,(prefix,fields,identity) in specs.items():
 t=contracts[name]; ok("- fields (in order): "+", ".join(fields) in t.replace(chr(96),"") and (identity is None or identity in t),"readable contract identity mismatch")
def cases(name,prefix):
 ...
 def parse(body):
  ...
  return d
 out=[(i,k,parse(b)) for i,k,b in blocks]; ok([i for i,_,_ in out]==[f"{prefix}-{k}{n:02d}" for k in ("P","N") for n in range(1,11)],f"{name} corpus inventory mismatch"); return out
def add(name,i,kind,observed): results.setdefault(name,[]).append({"case_id":i,"command":"deterministic-packaged-skill-corpus","selected_skill":name,"expected_classification":kind,"observed_deterministic_result":observed,"terminal_outcome":"PASS"})
for name,(prefix,_,_) in specs.items():
 for i,kind,f in cases(name,prefix):
  if name=="artifact-refresh":
   expected=("NO_CHANGE" if f["artifact"]==f["governing_source"] else "UPDATED") if kind=="POSITIVE" else "HANDOFF_REQUIRED"; ok(f["expected_outcome"]==expected and ((f["handoff_reason"]=="NONE")== (kind=="POSITIVE")) and ((f["removed"]!="<empty>")== (expected=="UPDATED")),"artifact corpus result mismatch"); observed={"outcome":f["expected_outcome"],"artifact":f["expected_artifact"],"removed":({} if f["removed"]=="<empty>" else dict(x.split("=",1) for x in f["removed"].split(", ")))}
  elif name=="blind-read":
   ok((kind=="POSITIVE" and all(x in f for x in ("expected_purpose","expected_unresolved_reference","expected_action_blocker"))) or (kind=="NEGATIVE" and f.get("expected_result","").startswith("HANDOFF_REQUIRED:")),"blind corpus result mismatch"); observed=f["expected_result"] if kind=="NEGATIVE" else {"interpreted_purpose":f["expected_purpose"],"unresolved_reference":[] if f["expected_unresolved_reference"]=="<empty>" else [f["expected_unresolved_reference"]],"action_blocker":[] if f["expected_action_blocker"]=="<empty>" else [f["expected_action_blocker"]]}
  else:
   ok((kind=="POSITIVE" and all(f"expected_{x}" in f for x in specs[name][1])) or (kind=="NEGATIVE" and f.get("expected_result")=="HANDOFF_REQUIRED"),"project corpus result mismatch"); observed=f["expected_result"] if kind=="NEGATIVE" else {x:([f[f"expected_{x}"]] if x in ("changes_since_touch","evidence_handle") else f[f"expected_{x}"]) for x in specs[name][1]}
  add(name,i,kind,observed)
...
PY
cat "$RECEIPT"
printf '%s\n' "A02 TERMINAL PASS frozen_head=$EXPECTED_SOURCE_SHA body_sha256=8f425cf99359cbcd8348b3d6b03c44272805b3308c155de79a2750818362d0e6 body_updated_at=2026-08-30T07:46:36Z portfolio_n=6 admitted=plan-stress-test,frame-check,decision-rationale,artifact-refresh,blind-read,project-brief UNCLASSIFIED=0 reviewer=codexy-sentinel"
