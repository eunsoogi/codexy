...
pub(crate) fn has_bare_no_fallback_field_without_availability(line: &str) -> bool {
    [
        "no fallback route:",
        "no fallback-route:",
        "no fallback path:",
...
pub(crate) fn defect_capture_clause(line: &str) -> Option<&str> {
...
pub(crate) fn has_tool_name(line: &str, tool: &str) -> bool {
    line.contains(tool) || line.contains(&format!("codex_app.{tool}"))
}

pub(crate) fn has_handler_marker(line: &str) -> bool {
    let normalized = line.to_ascii_lowercase();
    [
